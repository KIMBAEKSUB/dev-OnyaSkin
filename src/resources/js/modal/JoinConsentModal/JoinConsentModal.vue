<template>
    <div v-if="active" @click.self="closeJoinConsentModal" class="modal_background">
        <div class="modal_01 fs_discription_02 fc_gray_04" style="padding: 1.5rem">
            <span class="material-icons close_icon fc_gray_05" @click="closeJoinConsentModal">close</span>
            <div class="py_15 text-center fs_title_02 fw_600">Contract Consensus</div>
            <div>
                <div class="join_agree">
                    <input type="checkbox" id="join_agree" v-model="consentToAll"><div class="check_box"></div>
                    <label class="fw_600" for="join_agree">I agree with everyone.</label>
                </div>
                <div>
                    <input type="checkbox" id="join_agree_1" v-model="consentToTerms"><div class="check_box"></div>
                    <label class="fw_600" for="join_agree_1">Consent to Terms of Use <span class="fc_red_01">(Essential)</span></label>
                </div>
                <div class="Terms">
                    제1조 목적<br>
                    <br>
                ...
                <div>
                    <input type="checkbox" id="join_agree_2" v-model="contentToUse"><div class="check_box"></div>
                    <label class="fw_600" for="join_agree_2">Consent to use personal information <span class="fc_red_01">(Essential)</span></label>
                </div>
                <div class="Terms">
                ...
                <div class="d_flex_space_between">
                    <button style="width:49.5%" class="button_03" @click="closeJoinConsentModal">Cancel</button>
                    <button style="width:49.5%" class="button_01" @click="openJoinModal">Join</button>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    watch: {
        consentToAll(val){
            this.consentToTerms = val;
            this.contentToUse = val;
        }
    },
    data () {
        return {
    ...
    methods: {
        closeJoinConsentModal() {
            this.$emit('closeJoinConsentModal');
        },
        openJoinModal() {
            if(this.isConsentedAllTerms()){
                this.displayMessage();
                return ;
            }
            this.$emit('openJoinModal');
        },
        isConsentedAllTerms() {
            return  !this.consentToTerms || !this.contentToUse;
        },
        displayMessage(){
            alert('You can sign up only if you agree to the terms and conditions of use and the personal information processing policy.');
        }
    }
}
</script>
